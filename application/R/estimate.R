...
fitnuisance <- function(data, nfolds,
    ...
    ## extract data
    Y <- as.numeric(data[["Y"]])
    Z <- as.data.frame(data[, substr(names(data), 1, 1) == "Z"])
    L <- as.numeric(data[["L"]])
    A <- as.numeric(data[["A"]])
    W <- as.data.frame(data[, substr(names(data), 1, 1) == "W"])
...
# construct estimators of direct and indirect effects
estimators <- function(data, delta, nuisance,
                       sl_lib = c("SL.myglmnet", "SL.myglm", "SL.mean")) {
    ## set SL candidates
    ...
    d11  <- truncate(predict(nuisance$fitd,
                             newdata = data.frame(Z, A = 1, W))$pred[, 1])
    d10  <- truncate(predict(nuisance$fitd,
                             newdata = data.frame(Z, A = 0, W))$pred[, 1])
    m11  <- predict(nuisance$fitm,
                    newdata = data.frame(Z, L = 1, A = 1, W))$pred[, 1]
    m10  <- predict(nuisance$fitm,
                    newdata = data.frame(Z, L = 1, A = 0, W))$pred[, 1]
    m01  <- predict(nuisance$fitm,
                    newdata = data.frame(Z, L = 0, A = 1, W))$pred[, 1]
    m00  <- predict(nuisance$fitm,
                    newdata = data.frame(Z, L = 0, A = 0, W))$pred[, 1]
    ...
    osde <- mean(eifD)
    osie <- mean(eifI)
    seosde <- sd(eifD) / sqrt(length(A))
    seosie <- sd(eifI) / sqrt(length(A))
    ...
    ## iterative TMLE
    while (!stopcrit) {
        # fit logistic tilting models
        tiltm <- glm(Y ~ 0 + offset(qlogis(bound(m))) + HD + HI,
                     family = binomial())
        tiltb <- glm(L ~ 0 + offset(qlogis(bound(b1A))) + KD + KI,
                     family = binomial())
        tiltg <- glm(A ~ 0 + offset(qlogis(bound(g1))) + MD + MI,
                     family = binomial())

        # check tilting model coefs and set to zero if NA
        if (any(is.na(coef(tiltm)))) {
          tiltm$coefficients[is.na(coef(tiltm))] <- 0
        }
        if (any(is.na(coef(tiltb)))) {
          tiltb$coefficients[is.na(coef(tiltb))] <- 0
        }
        if (any(is.na(coef(tiltg)))) {
          tiltg$coefficients[is.na(coef(tiltg))] <- 0
        }


        # predict from tilting models
        ...
        m11 <- plogis(qlogis(bound(m11)) + coef(tiltm)[1] * HD11 +
                      coef(tiltm)[2] * HI11)
        m10 <- plogis(qlogis(bound(m10)) + coef(tiltm)[1] * HD10 +
                      coef(tiltm)[2] * HI10)
        m01 <- plogis(qlogis(bound(m01)) + coef(tiltm)[1] * HD01 +
                      coef(tiltm)[2] * HI01)
        m00 <- plogis(qlogis(bound(m00)) + coef(tiltm)[1] * HD00 +
                      coef(tiltm)[2] * HI00)
        ...
        stopcrit <- max(abs(c(mean(eifDp), mean(eifIp)))) <
            max(c(sd(eifDp), sd(eifIp))) *
                log(length(A)) / length(A) | iter > 6
    }
    ...
    de_est <- ubar1 * g1 + ubar0 * (1 - g1) - (u1 * g1delta + u0 *
                                               (1 - g1delta))
    ie_est <- (u1 - ubar1) * g1delta + (u0 - ubar0) * (1 - g1delta)

    # influence functions for direct and indirect effect
    eifD <- HD * (Y - m) + KD * (L - b1A) + MD * (A - g1) + (uA - ubarA) +
        de_est
    eifI <- HI * (Y - m) + KI * (L - b1A) + MI * (A - g1) + J * (uA - ubarA) +
        ie_est

    tmlede <- mean(de_est)
    tmleie <- mean(ie_est)
    setmlede <- sd(eifD) / sqrt(length(A))
    setmleie <- sd(eifI) / sqrt(length(A))
